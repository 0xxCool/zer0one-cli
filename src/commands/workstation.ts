...
type WsRow = {
  ...
  nodeStartedAt: string | null;
};

async function findActive(): Promise<WsRow | null> {
  ...
  // response for the id.
  const spawnRes = await api<{
    ok: boolean;
    id?: string;
    state?: string;
    error?: string;
  }>("/api/workstation/spawn", { method: "POST", body: { gpuTier: "a4000" } })
    .catch((e) => ({ ok: false, error: (e as Error).message }));

  if ("id" in spawnRes && spawnRes.id) {
    const detail = await api<{ ok: boolean; workstation: WsRow }>(
      `/api/workstation/${spawnRes.id}`,
    );
...
export async function workstationOpenCommand(opts: { tier: string }) {
  const spawn = await api<{
    ok: boolean;
    id?: string;
    state?: string;
    error?: string;
    accessToken?: string;
  }>("/api/workstation/spawn", { method: "POST", body: { gpuTier: opts.tier } });

  if (spawn.error === "workstation_already_active" && spawn.id) {
    console.log(`ℹ workstation already active: ${spawn.id} (state=${spawn.state})`);
    console.log(
      `  Open it in your browser:  https://app.zer0onelab.com/de/workstation`,
    );
    return;
  }

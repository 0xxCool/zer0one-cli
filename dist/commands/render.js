...
function parseFrameRange(s) {
    const m = /^(\d+)-(\d+)$/.exec(s);
    if (!m)
        throw new Error(`--frames must be N-M (got ${s})`);
...
async function uploadViaPresigned(url, filePath, size) {
...
export async function renderCommand(args) {
    ...
    if (head.subarray(0, 7).toString() !== "BLENDER" && !filename.endsWith(".zip") && !filename.endsWith(".tar.gz")) {
        console.warn(`⚠ ${filename} does not start with a BLENDER magic number — server may reject.`);
    }
    // 2. Draft job
    const draft = await api("/api/render/jobs", { method: "POST", body });
    const jobId = draft.job.id;
    console.log(`✓ draft job created: ${jobId}`);
    console.log(`  estimate: ~${draft.estimate.estimatedSeconds}s, ~$${(draft.estimate.estimatedCents / 100).toFixed(2)}`);
    // 3. Get presigned upload URL
    const pres = await api(`/api/render/jobs/${jobId}/presign-upload`, { method: "POST", body: { filename } });
    ...
    let prev = "queued";
    for (;;) {
        const list = await api(`/api/render/jobs`);
